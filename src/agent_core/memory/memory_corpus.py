...
class MemoryCorpus:
    ...
    def search(self, query: str, top_n: int = 5) -> List[dict]:
        ...
        Returns:
            [{"path": str, "snippet": str, "source": "corpus"}, ...]
        """
        results: List[dict] = []
        ...
        if self._qmd_enabled:
            for hit in self._search_qmd(query, top_n):
                ...
                if not path or not self._path_in_corpus(path):
                    continue
                if path in seen_paths:
                    continue
                seen_paths.add(path)
                snippet = str(hit.get("snippet", hit.get("content", "")))[:300]
                results.append({"path": path, "snippet": snippet, "source": "corpus"})
                if len(results) >= top_n:
                    return results

        if len(results) < top_n:
            for path, snippet in self._search_keyword(query, top_n):
                sp = str(path)
                if sp in seen_paths:
                    continue
                seen_paths.add(sp)
                results.append(
                    {"path": sp, "snippet": snippet[:300], "source": "corpus"}
                )
                if len(results) >= top_n:
                    break
        return results
    ...
    def _search_keyword(self, query: str, top_n: int) -> List[Tuple[Path, str]]:
        ...
        return [(path, snippet) for _, path, snippet in scored[:top_n]]
